...
class AshbyClientTest {
    ...
    void fetchesAndParsesBoard() throws Exception {
        ...
        Company company = Company.register("Linear", AtsSource.ASHBY, "linear");
        ...
        assertThat(request.getPath()).isEqualTo("/posting-api/job-board/linear");
    }

    @Test
    void sourceIsAshby() {
        assertThat(client.source()).isEqualTo(AtsSource.ASHBY);
    }

    @Test
    void emptyJobsYieldsNoPostings() throws Exception {
        server.enqueue(new MockResponse().setBody("{\"jobs\":[]}")
                .addHeader("Content-Type", "application/json"));
        Company company = Company.register("Empty", AtsSource.ASHBY, "empty");
